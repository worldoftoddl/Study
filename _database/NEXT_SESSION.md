...
### 15. 적용사례(IE) 분리: 6개 기준서 완료 (2026-03-18)
- **DOCX 재추출** (깨진 테이블 복원):
  - 1001호 재무제표 표시 (423줄, 26/26 금액 검증 통과)
  - 1007호 현금흐름표 (271줄, 5/5 금액 검증 통과)
  - 1008호 회계정책 (100줄, 27/27 금액 검증 통과)
  - 1012호 법인세 (784줄, 215/215 금액 검증 통과) — 8개 사례, 대규모
- **MD 직접 분리** (테이블 정상):
  - 1019호 종업원급여 (56줄, 문단비교표만)
  - 1021호 환율변동효과 (74줄, 텍스트만)
- **원본 MD에서 적용사례 섹션 제거**: 총 약 2,000줄 삭제
- **과도한 빈 줄 정리**: 전체 적용사례 파일에 `\n{3,}` → `\n\n` 적용 (267줄 제거)
- **`/적용사례분리` 슬래시 명령어**: 4단계에 빈 줄 정리 규칙 추가
- **`pipeline/extract_ie_tables.py`**: DOCX 적용사례 테이블 추출 참조 스크립트

### 16. md_section_cleaner.py 치명적 버그 발견 및 복구 (2026-03-18)
- **버그 원인**: 규칙 1(`## 시행일*` 삭제)이 시행일 헤더부터 다음 `## ` 헤더까지 전부 삭제
  - 7개 기준서에서 `## 시행일*` 이후에 부록 A~E, B문단(적용지침), 적용사례가 `## ` 헤더 없이 이어져 있어서 시행일과 함께 삭제됨
  - 규칙 3(`### 기타 참고사항` 삭제)에서도 `### ` 헤더에서 멈추지 않아 후속 섹션까지 삭제
  - 두 번째 `## 시행일*` 섹션(결론도출근거 내부)이 BC문단까지 삭제
- **영향 범위**: 23개 파일, 부록 11개 파일 손실, B문단 7개 파일 손실, BC문단 14개 파일 손실
  - 최악: 1101(부록A~E 전체), 1102(B1~B61+BC183개), 1103(B1~B69), 1109(BC383개)
- **복구 절차**:
  1. `output/docx_md/` 를 `git checkout 011f159` 로 cleaner 적용 전 상태로 롤백
  2. `md_section_cleaner.py` 버그 수정:
     - 규칙 1: `^부록`, `^[A-E]\d`, `^정의된 용어`, `^BC[A-Z]?\d` 패턴에서 삭제 중단
     - 규칙 3: `### ` 헤더에서도 break하도록 수정
  3. 수정된 cleaner 재실행: 63개 파일, **6,521줄 삭제** (이전 12,244줄 대비 5,723줄 차이 = 이전에 잘못 삭제된 콘텐츠)
- **검증 결과**: 전체 63개 파일, B문단·BC문단·부록 모두 원본(011f159)과 동일 — 손실 0건 ✓
- **적용사례 분리 작업 롤백**: 이전 세션(#15)과 이번 세션에서 분리했던 적용사례 파일(38개) 전량 롤백됨. 다음 세션에서 재실행 필요.

### 17. 적용사례(IE) 전체 분리: 43개 파일 완료 (2026-03-18)
- **`pipeline/batch_extract_ie.py` 신규 생성** — 적용사례 일괄 분리 배치 스크립트
  - `--dry-run`: 미리보기, `--single NUM`: 특정 기준서만 처리
  - 종료 마커: `## 결론도출근거` 또는 `## 본 문` (중복 구조 대응)
  - 1열 테이블(깨진 테이블 후보) 자동 탐지 + 보고
  - 멱등성: 재실행 시 0개 처리
- **처리 결과**:
...
|------|------|
| 처리 파일 | **43개** |
| 적용사례 없는 파일 | 20개 (1002, 1010, 1016, 1020, 1023, 1026, 1029, 1040, 1106, 1112 등) |
| 추출된 적용사례 | **14,531줄** |
| 원본에서 제거 | **15,304줄** (1033호 중복 구조 포함) |
| 테이블 총 | 906개 |
| 출력 경로 | `output/docx_md/적용사례/IE_K-IFRS_*.md` (43개 파일) |

- **주요 파일 (줄 수 상위)**:
  - 1109호 금융상품: 1,651줄 (115개 테이블)
  - 1115호 수익: 1,295줄 (34개 테이블)
  - 1103호 사업결합: 947줄 (35개 테이블)
  - 1012호 법인세: 897줄 (93개 테이블)
  - 1032호 금융상품 표시: 846줄 (90개 테이블)
- **특수 케이스 처리**:
  - 1033호 중복 구조: 2개 범위 모두 제거, 첫 번째 콘텐츠만 IE 파일에 보존 (729줄)
  - 1007호·1034호·1037호·2112호: `## 부록 A/B/C` 헤더가 적용사례 내부 → 종료 마커를 `## 결론도출근거`만으로 한정하여 해결
  - 경영진설명서·실무서2: 파일 대부분이 적용사례 → 정상 분리
- **테이블 품질 진단**: 28개 파일에서 1열 테이블 감지, 분석 결과 95%가 의도적 단일열(목차, 단위 표시 등) 또는 데이터 보존된 압축 포맷 → DOCX 재추출 불필요
- **검증**:
  - 원본 `## 적용사례` 잔존: 0개 ✓
  - `## 결론도출근거` 보존: 62개 파일 ✓
  - 멱등성: 재실행 시 0개 처리 ✓
...
### 4. 저작권/목차 정제 재실행
- `011f159` 커밋에서 수행했던 저작권 블록 삭제, 목차 테이블 분리를 다시 적용해야 함
- 이전 `/적용사례분리` 슬래시 명령어와 배치 스크립트 로직 재사용 가능

### 5. 적용사례(IE) few-shot 분리
...
- `pipeline/extract_ie_tables.py` — DOCX 적용사례 테이블 추출 참조 스크립트 (1001호 전용)
- `pipeline/batch_extract_ie.py` — 적용사례 일괄 분리 배치 스크립트 (전체 43개 파일)
...
- `output/docx_md/*.md` — DOCX→MD 변환 결과 (66개, 시행일/의결문/기타참고사항 삭제 완료, 부록·BC 보존)
- `output/docx_md/_TOC_*.md` — 목차 별도 저장 (3개)
- `output/docx_md/적용사례/IE_K-IFRS_*.md` — 적용사례 분리 파일 (**43개**, 14,531줄)
