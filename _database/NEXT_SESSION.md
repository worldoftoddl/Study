...
### 5. 적용사례(IE) 분리 재실행
- 이전 세션에서 38개 파일 분리 완료했으나 docx_md 롤백으로 전량 소실
- 배치 스크립트 로직 보존되어 있으므로 재실행 가능
- DOCX 재추출 필요: 1028, 1107, 1113, 1114 (금액 열 누락)
- 1033호 특수구조(중복 구조) 별도 처리 필요

### 6. 적용사례(IE) few-shot 분리
...
- `output/docx_md/적용사례/` — **미생성** (다음 세션에서 재실행 필요)
